        ...
        <div className="grid grid-cols-5 gap-5 mt-16 text-2xl font-bold">
          {Array.from({ length: 25 }, (_, i) =>
            tableOccupied.some(
              ...
              <p
                key={i}
                onClick={() => changeStatus(i + 1, "unoccupied")}
                className="p-2 rounded-xl bg-foreground text-white h-40 content-center cursor-pointer"
              >
                Table {i + 1}: OCCUPIED
              </p>
            ) : (
              <button
                key={i}
                onClick={() => changeStatus(i + 1, "occupied")}
